...
> Document de référence — compagnon de `Cert_EET_Dossier_Conception_DiagrammeClasses.md`
> et `Cert_EET_DiagrammeClasses_Description.md`
> Version 1.2 · Juin 2026 · Iyed Omri · PFE ESPRIT — Ingénierie Logicielle
>
...
**Objet-valeur (value object)** — Objet défini **uniquement par ses valeurs**, sans identité propre ni cycle de vie indépendant ; il n'a de sens qu'au sein de l'objet qui le contient, et il est immuable. *Dans Cert_EET :* `EmpreinteSHA256`.
...
**Instantané (snapshot)** — Copie figée de l'état de certaines données à un moment précis, prise pour être utilisée puis jetée (non maintenue à jour). *Dans Cert_EET :* les fragments de contexte assemblés lors d'une génération IA sont des données ponctuelles non persistées.
...
**SSE (Server-Sent Events)** — Technologie permettant au serveur de pousser des données vers le navigateur en flux continu, sans que le client ne re-demande. *Dans Cert_EET :* utilisable pour notifier en temps réel la fin d'une génération IA ou d'une indexation.

**LLM (Large Language Model)** — Grand modèle de langage (ex. GPT-4o, Claude, Mistral) générant du texte. *Dans Cert_EET :* moteur de la génération de questions assistée (F4), interchangeable via une couche d'abstraction.

**RAG (Retrieval-Augmented Generation)** — Technique consistant à **augmenter** le prompt envoyé au LLM avec des passages de documents pertinents récupérés en temps réel, au lieu de ré-entraîner le modèle. La génération est ainsi ancrée dans un contenu spécifique et vérifiable, non inventé. *Dans Cert_EET :* l'IA génère des questions de QCM en s'appuyant sur les `SupportPedagogique` indexés du module cible ; les questions reflètent le vrai contenu du cours.

**Embedding (vectorisation)** — Transformation d'un texte en un vecteur numérique de haute dimension qui capture sa signification sémantique. Deux textes proches en sens ont des vecteurs proches. *Dans Cert_EET :* les fragments de documents de cours sont vectorisés lors de l'indexation, puis la question du formateur est vectorisée à son tour pour retrouver les passages les plus pertinents.

**Base vectorielle (vector store)** — Base de données spécialisée dans le stockage et la recherche de vecteurs par similarité sémantique (recherche par « proximité »), par opposition à la recherche exacte d'une base relationnelle. *Dans Cert_EET :* l'extension **pgvector** de PostgreSQL est utilisée, ce qui évite toute infrastructure supplémentaire.

**Chunk (fragment de document)** — Morceau de texte obtenu en découpant un document source avant vectorisation. La taille du chunk (quelques centaines de tokens) est un paramètre clé : trop petits, les fragments perdent leur contexte ; trop grands, ils noient l'information pertinente. *Dans Cert_EET :* les `SupportPedagogique` sont découpés en chunks lors de l'indexation asynchrone.
...
**Différenciateur (fonctionnalité différenciante)** — Caractéristique absente de la concurrence qui distingue le produit. *Dans Cert_EET :* la génération de questions assistée par IA (F4) et la détection de décrochage proactive côté formateur.

**Distracteur** — Dans un QCM, option de réponse **incorrecte mais plausible**, conçue pour distinguer les apprenants qui maîtrisent le sujet de ceux qui ne le maîtrisent pas. Un bon distracteur correspond à une erreur classique ou à une confusion fréquente. *Dans Cert_EET :* la génération IA de `SuggestionQuestionIA` inclut des distracteurs proposés, qui constituent la valeur ajoutée principale de l'assistant (les rédiger manuellement est la partie la plus fastidieuse de la création de QCM).

**Humain dans la boucle (Human-in-the-loop)** — Principe d'architecture IA selon lequel un humain valide ou corrige les décisions du modèle avant qu'elles ne produisent un effet réel. Garantit la qualité et la responsabilité humaine sur les résultats. *Dans Cert_EET :* toute `SuggestionQuestionIA` doit être explicitement acceptée par le formateur via `accepter()` avant d'intégrer la `BanqueQuestions`. Aucune suggestion n'est publiée automatiquement.

**Décrochage** — Désengagement progressif d'un apprenant menant à l'abandon. *Dans Cert_EET :* détecté par analyse automatique des données d'activité (absences, échecs, retards), générant une `AlerteDecrochage` au formateur responsable (UC8 Formateur).
...
**Contexte (injection de contexte)** — Données supplémentaires fournies au LLM dans le prompt pour adapter ses réponses à une situation précise. *Dans Cert_EET :* lors d'une génération IA, le contexte est constitué des fragments de `SupportPedagogique` récupérés par le RAG et des paramètres du formateur (module, niveau, type de questions).
...
Abstraction · Abstraction prématurée · Access token · Acteur · Agrégation · Anonymisation · Anti-fraude · Anti-plagiat · API REST · Association · Association binaire · Asynchrone · Attribut · Attribut dérivé · Audit log · Authentification · Autorisation · Base de données relationnelle · Base vectorielle · bcrypt · Benchmark · Bull · Cardinalité · Cascade · Cert_EET · Certification certifiante · Chunk · Classe · Classe abstraite · Classe d'association · Clé étrangère · Clé primaire · Cohésion · Colonne discriminante · Compartiment · Composition · Conteneurisation · Contexte · Contrôleur · Couplage · CPU-bound · Critère · Cycle de vie · Décrochage · Délégation · Dépendance · Diagramme de cas d'utilisation · Diagramme de classes · Diagramme d'états-transitions · Diagramme de séquence · Différenciateur · Distracteur · Docker · Domain Model · DRY · DTO · Embedded value · Embedding · Empreinte cryptographique · Encapsulation · Endpoint · Énumération · Entité métier · Entité (ORM) · Event loop · File de tâches · Généralisation · GRASP · Grille de notation · Hachage · Héritage · Humain dans la boucle · Immuabilité · Information Expert · Instantané · Intégrité · Invariant métier · I/O · JWT · Lacune G3 · LLM · LSP · Méthode · Méthode abstraite · Micro-certification · Minimisation des données · Module (NestJS) · MFA / double authentification · Multiplicité · Navigabilité · NestJS · Niveau de performance · Node.js · Objet-valeur · onDelete · ORM · Persistance · Polymorphisme · Pondération · POO · PostgreSQL · Proctoring · Programmation orientée objet · QR code · RAG · Randomisation · Rate limiting · RBAC · Redis · Refresh token · Relation N–N · Repository · Responsabilité · Révocation · RGPD · Scoring · Séparation des préoccupations · Service · Seuil de validation · SHA-256 · Single Table Inheritance · SOLID · SRP · SSE · Stéréotype · Synchrone · Table · Table de liaison · TLS · TypeORM · UML · UUID · Value object · Verrouillage progressif · Visibilité · Worker · Worker Thread

---

*Fin du document — Glossaire des termes techniques · Cert_EET v1.2*
