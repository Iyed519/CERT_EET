...
- *Génération IA* : le formateur peut lancer une génération assistée par IA pour obtenir des suggestions de questions — voir **UC10**.
...
**Déclencheur** : Le **système d'analyse de décrochage** (cf. F4 du CdC) détecte un pattern de risque chez un apprenant et émet une alerte au formateur responsable.

**Critères de déclenchement** (analysés automatiquement par la plateforme) :
...
**Caractère différenciant** : aucune des plateformes du benchmark ne propose ce mode de suivi proactif assisté par analyse automatique des données d'apprentissage.
...
## UC10 — Générer des questions de QCM assistées par IA

**Objectif métier** : Accélérer la création de questions de QCM de qualité en utilisant un assistant IA qui génère des propositions ancrées dans les vrais supports de cours de la formation, que le formateur révise et valide avant publication.

**Déclencheur** : Depuis l'espace d'une formation dont il est responsable, le formateur clique sur **"Générer des questions avec l'IA"** — accessible depuis l'interface de création de QCM (UC1) ou directement depuis la banque de questions.

**Prérequis** : au moins un **support pédagogique** (PDF, PPTX, document texte) doit avoir été uploadé et indexé pour la formation ou le module ciblé.

**Déroulement nominal** :
1. Le formateur sélectionne le **module ou la formation** sur lequel l'IA doit se baser.
2. Il choisit les **paramètres de génération** :
   - **Nombre de questions** à générer (ex : 5, 10, 20)
   - **Type de questions** : QCM, QCU, Vrai/Faux, ou mix
   - **Niveau de difficulté** cible : Débutant / Intermédiaire / Avancé
   - **Focus thématique** optionnel (ex : « uniquement sur la partie sécurité JWT »)
3. Il clique sur **"Lancer la génération"**. Le système envoie les supports indexés et les paramètres au LLM.
4. La génération s'exécute (quelques secondes à quelques dizaines de secondes) — un indicateur de progression est affiché.
5. Le système affiche les **suggestions générées**, une par une. Pour chaque suggestion :
   - **Énoncé proposé** par l'IA
   - **Options de réponse** proposées (dont les distracteurs — les mauvaises réponses plausibles)
   - **Bonne(s) réponse(s)** identifiée(s)
   - **Explication** proposée (justification pédagogique)
   - **Score de confiance** de l'IA (indicatif)
6. Le formateur **révise chaque suggestion** :
   - ✅ **Accepter** : la suggestion est convertie en vraie `Question` et ajoutée à la `BanqueQuestions` de la formation.
   - ✏️ **Modifier puis accepter** : le formateur édite l'énoncé, les options ou l'explication avant d'accepter.
   - ❌ **Rejeter** : la suggestion est archivée (non publiée) — le rejet est tracé pour améliorer les générations futures.
7. Le formateur peut **relancer une génération** avec des paramètres différents sur le même module.

**Règle fondamentale — Humain dans la boucle** : aucune suggestion IA n'est jamais publiée automatiquement dans un QCM. Toute question issue de l'IA **doit passer par la validation explicite du formateur** avant d'être utilisable. Cette règle est non contournable.

**Ancrage RAG — pourquoi les questions sont fiables** : l'IA ne génère pas « dans le vide ». Elle s'appuie sur les **supports pédagogiques indexés** (PDF, PPTX uploadés par le formateur) pour ancrer ses questions dans le vrai contenu enseigné. Une question générée sur « NestJS – Module 3 » porte sur ce qui est réellement dans le cours, pas sur ce que Wikipedia dit de NestJS.

**Traçabilité complète** :
- Chaque lot de génération est enregistré (`GenerationIA`) : formateur, module source, modèle LLM utilisé, paramètres, date.
- Chaque suggestion est archivée avec son statut final (Acceptée / Modifiée / Rejetée).
- Une question acceptée conserve la mention de son origine IA (`origine : IA`) dans la banque.

**Cas alternatifs** :
- *Support non indexé* : si aucun support n'est disponible pour le module sélectionné, le système propose d'en uploader un avant de lancer la génération.
- *Génération échouée* : en cas d'erreur LLM ou de timeout, le système notifie le formateur et lui propose de réessayer.
- *Qualité insuffisante* : si toutes les suggestions d'un lot sont rejetées, le formateur peut relancer avec un focus thématique plus précis.

**Cas d'usage typique** : un formateur prépare un QCM de révision de 20 questions sur le Module 5. Au lieu de les rédiger toutes, il génère 30 suggestions en 30 secondes, en garde 18 telles quelles, en modifie 2, et en rejette 10. Il économise plusieurs heures de rédaction tout en gardant le contrôle pédagogique total.

---

## UC11 — Gérer les supports pédagogiques d'une formation

**Objectif métier** : Uploader et organiser les documents de cours qui servent de source à l'assistant IA de génération de questions.

**Déclencheur** : Depuis l'espace d'une formation, le formateur clique sur **"Supports de cours"**.

**Déroulement nominal** :
1. Le formateur sélectionne le **module** auquel rattacher le support.
2. Il uploade le fichier (PDF, PPTX, TXT, MD — taille maximale définie par l'administrateur).
3. Le système **indexe automatiquement** le document (découpage en chunks + génération des vecteurs) — opération asynchrone indiquée par un statut « En cours d'indexation ».
4. Une fois l'indexation terminée (statut « Indexé »), le support est disponible pour les générations IA.
5. Le formateur peut **supprimer ou remplacer** un support — les questions déjà générées et validées depuis ce support ne sont pas affectées.

**Statuts d'un support** : En cours d'indexation → Indexé → Erreur d'indexation.
...
| Publier automatiquement une question générée par l'IA | Validation humaine obligatoire (principe « humain dans la boucle ») |

---

*Fin du document — UC Formateur · Cert_EET v2.0*
