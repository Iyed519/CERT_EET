...
Les autres requêtes attendent. Le chatbot ne répond plus. Les tableaux de bord ne se chargent plus. C'est ce qu'on appelle **bloquer l'event loop**.
...
- Appel à l'API OpenAI pour le chatbot.
...
> Node.js fonctionne avec un seul thread principal. L'asynchrone permet à ce thread de ne pas se bloquer pendant les attentes (base de données, API externe, fichiers). En revanche, pour un vrai calcul lourd comme générer un PDF ou calculer un hash, l'asynchrone seul ne suffit pas : le travail reste sur le thread principal. Pour ces cas, Cert_EET déporte le calcul ailleurs — soit sur une file Bull avec Redis (génération de certificats), soit sur un Worker Thread (traitements par lot).
...
*Cert_EET — Note technique annexe à l'ADR-02 — Mai 2026 — Iyed Omri*
