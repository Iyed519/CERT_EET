...
> Document de conception · Version 3.1 · Juin 2026
...
Le modèle comporte **36 classes** et **7 énumérations**, organisées en **8 packages**. Il totalise **41 associations et dépendances**, **3 généralisations** et **2 classes d'association**, l'ensemble étant justifié dans les sections suivantes.
...
**Choix retenu.** Les classes sont regroupées en huit packages correspondant aux domaines fonctionnels (Utilisateurs & Sécurité, Formations & Pédagogie, Évaluations QCM, Ateliers, Certification, Chatbot IA, Transverse, Énumérations).
...
| Relation | Mult. | Type | Justification |
|---|---|---|---|
| `Apprenant` — `ConversationChatbot` | `1` — `0..*` | Assoc. | Un apprenant peut ouvrir plusieurs fils de conversation (F4). |
| `ConversationChatbot` ◆— `MessageChatbot` | `1` — `1..*` | Compo. | Une conversation contient au moins un message. |
| `ConversationChatbot` ◇— `ContexteApprenant` | `1` — `0..*` | Agrég. | Le contexte est un instantané dérivé du profil, regroupé sans être une partie indissociable (§7). |
| `ContexteApprenant` ⇢ `Apprenant` | — | Dép. | Le contexte lit les données de l'apprenant pour se construire, sans référence permanente (§8). |
| `AlerteDecrochage` — `Apprenant` | `0..*` — `1` | Assoc. | Une alerte concerne un apprenant (UC8 Formateur). |
| `AlerteDecrochage` — `Formateur` | `0..*` — `1` | Assoc. | Une alerte notifie un formateur responsable (UC8 Formateur). |
...
Toutes vérifient un cycle de vie coïncident et une partie non partagée : `Formation*--Module`, `QCM*--ConfigurationAntiFraude`, `Question*--Option`, `TentativeQCM*--Reponse`, `GrilleNotation*--Critere`, `Critere*--NiveauPerformance`, `Livrable*--FichierLivrable`, `Correction*--Feedback`, `Certificat*--EmpreinteSHA256`, `ConversationChatbot*--MessageChatbot`, `SessionUtilisateur*--TokenJWT`. Dans chaque cas, la partie n'a de sens qu'au sein de son tout et n'est jamais partagée.
...
| `ConversationChatbot ◇— ContexteApprenant` | Instantané dérivé du profil, de nature à être regroupé sans constituer une partie indissociable. |
...
**Choix retenu.** `EmpreinteSHA256` et `ContexteApprenant` sont stéréotypés `«value object»`.

**Justification.** Le *Domain-Driven Design* distingue l'*entité* (identité suivie) de l'*objet-valeur* (défini par ses seuls attributs, immuable). `EmpreinteSHA256` est déterminée par la valeur du hash et n'a de sens qu'au sein du certificat ; `ContexteApprenant` est un instantané reconstructible injecté dans le LLM (F4).

**Conséquence.** `EmpreinteSHA256` se prête au patron *Embedded Value* (M. Fowler) : stockage dans les colonnes de la table propriétaire.

---

## 8. Dépendances et navigabilité

**Choix retenu.** `VisiteurPublic ⇢ VerificationCertificat` et `ContexteApprenant ⇢ Apprenant` sont des *dépendances* ; `Reponse → Question` et `Reponse → Option` sont des associations orientées.
...
| `construire(apprenant)` | `ContexteApprenant` | Fabrication de l'instantané (F4). |
...
| `«value object»` | `EmpreinteSHA256`, `ContexteApprenant` | Objets sans identité propre (§7) → immuabilité, *Embedded Value*. |
...
| `TypeQuestion` | QCM, QCU, VRAI_FAUX, REPONSE_COURTE, ASSOCIATION | Typologies F1 |
...
| Classe · attribut | Justification |
|---|---|
| `ConversationChatbot.mode` | Distingue le mode **Support** du mode **Suivi** ; ce dernier alimente la détection de décrochage. |
| `MessageChatbot.emetteur`, `horodatage` | Distinguent apprenant/assistant et ordonnent le fil. |
| `ContexteApprenant.profilFormation`, `scoresRecents`, `lacunesIdentifiees` | Données **injectées dans le LLM** pour contextualiser les réponses (F4). |
...
|---|---|---|
| Ajout | `VerificationCertificat` | Journalisation des consultations publiques (UC2/UC3 Visiteur) et comptage exposé à l'administrateur (UC5 Admin) ; sans elle, l'acteur `VisiteurPublic` serait isolé du modèle. |
| Ajout | `NoteCritere` | Classe d'association requise pour porter la note et le commentaire par critère (UC5 Formateur). |
| Suppression | relation `Formateur — Certificat` | Signature jugée institutionnelle (§13.1) ; remplacée par l'attribut `organismeEmetteur`. |
...
| F4 | Chatbot IA contextualisé | ConversationChatbot, MessageChatbot, ContexteApprenant, AlerteDecrochage |
...
| Entité vs objet-valeur ; Embedded Value | DDD ; M. Fowler | `EmpreinteSHA256`, `ContexteApprenant` (§7) |
...
| Traitement asynchrone | Architecture Node.js | Génération de certificat par file de tâches (§13.3) |
| Traçabilité des exigences | Ingénierie des besoins | Matrice de couverture F1–F7 (§14, §15) |

---

*Fin du dossier de conception du diagramme de classes · Cert_EET v3.1*
